"""Comprehensive test with Building permits to find addresses."""
...
async def test_building_comprehensive():
    """Test comprehensive extraction with Building permits."""
    logger.info("Testing comprehensive extraction with Building permits")
    logger.info("Building permits should have property addresses\n")
    
    scraper = create_accela_scraper(
        city_code="COSA",
        module="Building",  # Building permits should have addresses
        days_back=60,  # Look back further to get more permits
        max_pages=2,  # Check 2 pages
    ...
    valid_addresses = [p for p in permits if p.address and len(p.address.strip()) > 5 and 
                      not any(skip in p.address.lower() for skip in ['location', 'applicant:', 'n/a', 'none', 'tunnel', 'tops', 'company information', 'application type:', 'license type:'])]
    ...
    if total > 0:
        ...
        # Show permits with both
        if has_both > 0:
            both = [p for p in permits if (p.address and len(p.address.strip()) > 5) and (p.applicant_name and len(p.applicant_name.strip()) > 2)]
            logger.info(f"\n🎯 Permits with BOTH Address AND Applicant ({len(both)}/{total}):")
            for i, permit in enumerate(both[:3], 1):
                logger.info(f"\n{i}. {permit.permit_id}: {permit.applicant_name}")
                logger.info(f"   Address: {permit.address[:80]}")
    else:
        logger.warning("No permits extracted!")


if __name__ == "__main__":
    asyncio.run(test_building_comprehensive())
